{
 "cells": [
  {
   "cell_type": "markdown",
   "id": "70584333-f7d1-4502-b614-711f3039204a",
   "metadata": {},
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": 7,
   "id": "c76b418d-8307-4127-b596-160fa2722286",
   "metadata": {},
   "outputs": [
    {
     "name": "stdout",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": 8,
   "id": "e78e8102-aa12-4da5-85a5-60a9541613b8",
   "metadata": {},
   "outputs": [
    {
     "name": "stdout",
  ...
  {
   "cell_type": "markdown",
   "id": "3547d1ee-34f1-4729-8628-0b9df41e4d9b",
   "metadata": {},
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": 11,
   "id": "b22913e3-563c-4d3b-a536-aa0007f5729b",
   "metadata": {
    "scrolled": true
   },
   "outputs": [
   ...
   "source": [
    ...
    "noise_std_dev = 0.5  # Adjust this value to control the amount of noise\n",
    "data_cleaned['ALLSKY_SFC_SW_DWN_noisy'] = data_cleaned['ALLSKY_SFC_SW_DWN'] + np.random.normal(0, noise_std_dev, size=data_cleaned.shape[0])\n",
    "\n",
    "# Feature Engineering\n",
    "data_cleaned['DATE'] = pd.to_datetime(data_cleaned['DATE'])  # Convert 'DATE' column to datetime\n",
    "data_cleaned['Month'] = data_cleaned['DATE'].dt.month       # Extract month\n",
    ...
    "#  Define Features and Target\n",
    "features = ['TAVG', 'TMAX', 'TMIN', 'PRCP', 'SNOW', 'Month', 'Day', 'is_summer', 'is_winter']\n",
    "original_target = 'ALLSKY_SFC_SW_DWN'\n",
    "noisy_target = 'ALLSKY_SFC_SW_DWN_noisy'\n",
    "\n",
    "X = data_cleaned[features]\n",
    "y_original = data_cleaned[original_target]\n",
    "y_noisy = data_cleaned[noisy_target]\n",
    "\n",
    "# Train-Test Split\n",
    ...
    "rf_model_original = RandomForestRegressor(\n",
    "    n_estimators=500,\n",
    "    max_depth=20,\n",
    "    max_features='sqrt',\n",
    "    min_samples_split=10,\n",
    ...
    "    random_state=100\n",
    ")\n",
    "rf_model_noisy = RandomForestRegressor(\n",
    "    n_estimators=500,\n",
    "    max_depth=20,\n",
    "    max_features='sqrt',\n",
    "    min_samples_split=10,\n",
    ...
    ")\n",
    "\n",
    "rf_model_original.fit(X_train_orig, y_train_orig)\n",
    "rf_model_noisy.fit(X_train_noisy, y_train_noisy)\n",
    "\n",
    "# Predictions and Evaluation\n",
    "y_test_pred_orig = rf_model_original.predict(X_test_orig)\n",
    "y_test_pred_noisy = rf_model_noisy.predict(X_test_noisy)\n",
    "\n",
    "# Evaluate the original model\n",
    "test_rmse_orig = mean_squared_error(y_test_orig, y_test_pred_orig, squared=False)\n",
    "test_r2_orig = r2_score(y_test_orig, y_test_pred_orig)\n",
    "\n",
    "# Evaluate the noisy model\n",
    "test_rmse_noisy = mean_squared_error(y_test_noisy, y_test_pred_noisy, squared=False)\n",
    "test_r2_noisy = r2_score(y_test_noisy, y_test_pred_noisy)\n",
    "\n",
    "\n",
    "print(\"\\nModel Evaluation (Noisy Data):\")\n",
    "print(f\"  RMSE: {test_rmse_noisy:.4f}\")\n",
    "print(f\"  R² Score: {test_r2_noisy:.4f}\")\n",
    "\n",
    "\n"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": 12,
   "id": "08e272bd-29f6-45d6-b278-11fc33018280",
   "metadata": {},
   "outputs": [
    {
     "name": "stdout",
    ...
    },
    {
     ...
     "text": [
      "\n",
      "Predicted Solar Radiation (ALLSKY_SFC_SW_DWN_noisy): 6.75 kWh/m²/day\n",
      "Predicted Solar Energy Output: 1080303.52 kWh/day\n"
     ]
    },
    {
     "data": {
      "text/plain": [
       "(np.float64(6.7518970309295225), np.float64(1080303.5249487236))"
      ]
     },
     "execution_count": 12,
     "metadata": {},
   ...
   "source": [
    "def predict_solar_energy_with_location_noisy(model, data):\n",
    "\n",
    "    # Define the input features\n",
    "    feature_names = ['TAVG', 'TMAX', 'TMIN', 'PRCP', 'SNOW', 'Month', 'Day', 'is_summer', 'is_winter']\n",
    "    \n",
    ...
    "\n",
    "    # Display results\n",
    "    print(f\"\\nPredicted Solar Radiation (ALLSKY_SFC_SW_DWN_noisy): {predicted_radiation:.2f} kWh/m²/day\")\n",
    "    print(f\"Predicted Solar Energy Output: {solar_energy:.2f} kWh/day\")\n",
    "\n",
    "    return predicted_radiation, solar_energy\n",
    "\n",
    "\n",
    "# Test the model trained with noisy data\n",
    "print(\"\\n--- Testing Noisy Data Model ---\")\n",
    "predict_solar_energy_with_location_noisy(rf_model_noisy, data_cleaned)\n"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "42f3b5ad-d22b-4599-87f9-e808f8583f1b",
   "metadata": {},
   "outputs": [],
   "source": []
  }
